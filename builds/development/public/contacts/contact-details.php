...
if ($update==0 && !empty($_POST['note_text'])) {
	mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO notes (note_contact, note_text, note_date, note_status) VALUES 
		(
...
if ($update==1 && !empty($_POST['note_text'])) {
	mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE notes SET 
...
mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE history SET history_status = 2 WHERE history_contact = ".$_GET['id']."");
}
...
do {
	...
	mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE contacts SET contact_custom = '".$data."' WHERE contact_id = ".$_GET['id']."");
} while ($row_cfields = mysqli_fetch_assoc($cfields));
//
