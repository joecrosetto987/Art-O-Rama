...
global ${"totalRows_$name"};
	...
	${"query_$name"} = "$query";
	${"$name"} = mysqli_query($contacts, ${"query_$name"}) or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
	${"row_$name"} = mysqli_fetch_assoc(${"$name"});
...
function select_list($id,$rs,$matcha,$matchb,$v,$display) {
	...
	} while (${"row_$rs"} = mysqli_fetch_assoc(${"$rs"})); 
